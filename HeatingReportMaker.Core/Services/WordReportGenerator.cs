...
namespace HeatingReportMaker.Core.Services
{
    public class WordReportGenerator
    {
        public void GenerateReport(ApartmentHeating data)
        {
            ...
            string outputPath = Path.Combine(desktopPath, $"Расчет стоимости отопления {data.Address} кв. {data.ApartmentNumber} за {reportDate.ToLower()} г.docx");
            if (!File.Exists(templatePath))
            {
                throw new TemplateException("Шаблон для Word отчёта не был найден. Поместите шаблон ReportTemplate.docx в папку Templates в корне программы.");
            }
            try
            {
            ...
            {
                throw new TemplateException("Файл открыт в другой программе. Закройте его и повторите попытку.");
            }
            catch (UnauthorizedAccessException)
            {
                throw new TemplateException("Ошибка доступа к файлу.");
            }
        }
